...
def generate_config_from_args(args: argparse.Namespace) -> ModelMergerConfig:
    hf_subdir = os.path.join(args.local_dir, "huggingface")
    hf_model_config_path = hf_subdir if os.path.exists(os.path.join(hf_subdir, "config.json")) else args.local_dir
...
class BaseModelMerger(ABC):
    ...
    def __init__(self, config: ModelMergerConfig):
        ...
        candidate_paths = [self.hf_model_config_path]
        if self.config.local_dir and os.path.abspath(self.config.local_dir) != os.path.abspath(
            self.hf_model_config_path
        ):
            candidate_paths.append(self.config.local_dir)

        last_error = None
        for cfg_path in candidate_paths:
            try:
                self.model_config = AutoConfig.from_pretrained(
                    cfg_path, trust_remote_code=self.config.trust_remote_code
                )
                if cfg_path != self.hf_model_config_path:
                    warnings.warn(
                        f"Failed to load config from {self.hf_model_config_path}; "
                        f"fallback to {cfg_path}.",
                        stacklevel=2,
                    )
                    self.hf_model_config_path = cfg_path
                break
