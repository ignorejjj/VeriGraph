...
class FSDPModelMerger(BaseModelMerger):
    ...
    def _get_world_size(self) -> int:
        ...
        if not config_path.exists():
            # Fallback: infer world_size from shard filenames like model_world_size_4_rank_0.pt
            candidates = list(Path(self.config.local_dir).glob("model_world_size_*_rank_0.pt"))
            if len(candidates) == 0:
                raise FileNotFoundError(
                    f"Config file {config_path} does not exist, and no shard file "
                    "matching model_world_size_*_rank_0.pt was found."
                )
            match = re.search(r"model_world_size_(\d+)_rank_0\.pt$", candidates[0].name)
            if match is None:
                raise ValueError(f"Failed to infer world_size from shard filename: {candidates[0].name}")
