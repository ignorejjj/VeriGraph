...
NEGATIVE_TERMINATION_REASONS_FOR_TRAINING = {
    "missing_submit",
    "max_turns",
    "generation_length",
}
...
class AsyncEngine(sglang.srt.entrypoints.engine.Engine):
    def __init__(self, **kwargs):
        ...
        # Store a single event loop to prevent SGLang's internal handle_loop
        # from being stranded on a stale loop when a new one is created.
        self._event_loop = None

    def get_event_loop(self):
        """Return a consistent event loop for all async operations on this engine."""
...
def _to_1d_object_array(items: list) -> np.ndarray:
    """Create a 1-D object array where each element is a Python object.

    ``np.array(items, dtype=object)`` may infer a 2-D shape when the inner
    lists happen to share the same length, which breaks ``np.concatenate``
    across DP ranks that have different trajectory lengths.
    """
...
class SGLangRollout(BaseRollout):
    ...
    def _batch_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        with self.update_sampling_params(**kwargs):
            ...
            if self._tp_rank == 0:
                loop = self._engine.get_event_loop()
    ...
    def _is_verigraph_mode(self) -> bool:
        verigraph_cfg = self.config.multi_turn.get("verigraph", None)
        return bool(verigraph_cfg and verigraph_cfg.get("enable", False))
    ...
    def _build_verigraph_runtime(self, req: AsyncRolloutRequest) -> VeriGraphTrajectoryRuntime:
        from verl.utils.verigraph_rl import VeriGraphTrajectoryRuntime

        if not req.working_dir:
            raise ValueError("working_dir must be provided for VeriGraph multi-turn rollout.")

        verigraph_cfg = self.config.multi_turn.get("verigraph", {})
        timeout = verigraph_cfg.get("tool_execution_timeout_seconds", 120)
        if timeout is None:
            timeout = 120
        return VeriGraphTrajectoryRuntime(
            working_dir=req.working_dir,
            max_tool_output_chars=int(verigraph_cfg.get("max_tool_output_chars", 1200)),
            execution_timeout_seconds=float(timeout),
        )

    def _build_initial_verigraph_trajectory_info(self) -> Dict[str, Any]:
        return {
            "tool_stats": {
                "total_calls": 0,
                "successful_calls": 0,
    ...
    def _finalize_verigraph_trajectory_info(self, req: AsyncRolloutRequest) -> None:
        info = dict(req.trajectory_info or {})
        tool_stats = dict(info.get("tool_stats", {}))
        total_calls = int(tool_stats.get("total_calls", 0))
        successful_calls = int(tool_stats.get("successful_calls", 0))
        if req.is_valid_trajectory and total_calls > 0:
            process_reward = successful_calls / total_calls
        else:
            process_reward = 0.0
        tool_stats["process_reward"] = process_reward
        info["tool_stats"] = tool_stats
        info["valid_trajectory"] = int(req.is_valid_trajectory)
        req.trajectory_info = info

    async def _async_rollout_a_request_verigraph(
        self,
        ...
        **kwargs,
        ...
        from verl.utils.verigraph_rl import CODE_CALL_TAGS, CODE_RESULT_TAGS, extract_last_code_block

        _req = deepcopy(req)
        _req.trajectory_info = dict(_req.trajectory_info or self._build_initial_verigraph_trajectory_info())
        runtime = None
        verigraph_cfg = self.config.multi_turn.get("verigraph", {})
        trajectory_timeout = float(verigraph_cfg.get("trajectory_timeout_seconds", 1800))
        _trajectory_start = asyncio.get_event_loop().time()
        print(f"[DEBUG][verigraph] req_id={req.request_id} start, working_dir={getattr(req, 'working_dir', None)}", flush=True)
        try:
            runtime = self._build_verigraph_runtime(_req)
            print(f"[DEBUG][verigraph] req_id={req.request_id} runtime built ok", flush=True)
        except Exception as e:
            print(f"[DEBUG][verigraph] req_id={req.request_id} runtime build FAILED: {e}", flush=True)
            _req.is_valid_trajectory = 0
            _req.trajectory_info["valid_trajectory"] = 0
            _req.trajectory_info["termination_reason"] = "runtime_init_failed"
            self._finalize_verigraph_trajectory_info(_req)
            _req.finalize(self.tokenizer, {}, FinishReasonTypeEnum.STOP)
            return _req
        finish_reason_type = FinishReasonTypeEnum.STOP
        current_turns = 0
        max_turn_limit = max_turns if max_turns is not None else self.config.multi_turn.max_turns

        try:
            while current_turns < max_turn_limit or _req.state == AsyncRolloutRequestStateEnum.TOOL_CALLING:
                # Trajectory-level timeout: prevent one slow trajectory from
                # blocking the entire generation batch across all GPUs.
                elapsed = asyncio.get_event_loop().time() - _trajectory_start
                if elapsed > trajectory_timeout:
                    print(f"[WARNING][verigraph] req_id={req.request_id} trajectory timed out after {elapsed:.0f}s", flush=True)
                    _req.is_valid_trajectory = 0
                    _req.trajectory_info["valid_trajectory"] = 0
                    _req.trajectory_info["termination_reason"] = "trajectory_timeout"
                    finish_reason_type = FinishReasonTypeEnum.STOP
                    break
                print(f"[DEBUG][verigraph] req_id={req.request_id} turn={current_turns}/{max_turn_limit} state={_req.state}", flush=True)
                # 这里本身是初始化tool用的，我们没有对应的tool需要初始化
                if _req.state == AsyncRolloutRequestStateEnum.PENDING:
                    _req.state = AsyncRolloutRequestStateEnum.RUNNING
                    continue

                if _req.state == AsyncRolloutRequestStateEnum.TOOL_CALLING:
                    last_message = _req.messages[-1].content if _req.messages else ""
                    code = extract_last_code_block(last_message)
                    if not code:
                        _req.is_valid_trajectory = 0
                        _req.trajectory_info["valid_trajectory"] = 0
                        _req.trajectory_info["termination_reason"] = "missing_code_block"
                        _req.state = AsyncRolloutRequestStateEnum.COMPLETED
                        break

                    print(f"[DEBUG][verigraph] req_id={req.request_id} turn={current_turns} before aexecute, code_len={len(code)}", flush=True)
                    step_result = await runtime.aexecute(code.strip())
                    print(f"[DEBUG][verigraph] req_id={req.request_id} turn={current_turns} after aexecute, submitted={step_result.submitted_answer_present}", flush=True)
                    tool_stats = _req.trajectory_info["tool_stats"]
                    tool_stats["total_calls"] += 1
                    if step_result.call_result.get("error"):
                        tool_stats["failed_calls"] += 1
                    else:
                        tool_stats["successful_calls"] += 1

                    _req.trajectory_info["final_claims"] = list(step_result.final_claims)
                    _req.trajectory_info["submitted_answer_present"] = bool(step_result.submitted_answer_present)

                    observation = f"{CODE_RESULT_TAGS[0]}\n{step_result.observation}{CODE_RESULT_TAGS[1]}"
                    _req.add_user_response_messages(self.tokenizer, observation)

                    if len(_req.input_ids) >= self.config.max_model_len:
                        finish_reason_type = FinishReasonTypeEnum.LENGTH
                        _req.is_valid_trajectory = 0
                        _req.trajectory_info["valid_trajectory"] = 0
                        _req.trajectory_info["termination_reason"] = "max_model_len"
                        _req.state = AsyncRolloutRequestStateEnum.COMPLETED
                        break

                    if step_result.submitted_answer_present:
                        finish_reason_type = FinishReasonTypeEnum.STOP
                        _req.trajectory_info["termination_reason"] = "submitted_answer"
                        _req.state = AsyncRolloutRequestStateEnum.COMPLETED
                        break
                ...
                print(f"[DEBUG][verigraph] req_id={req.request_id} turn={current_turns} before engine_call, prompt_len={len(_req.get_generation_prompt_ids(self.tokenizer))}", flush=True)
                try:
                    output = await self._handle_engine_call(_req, do_sample, is_validate, **kwargs)
                except asyncio.TimeoutError:
                    _req.is_valid_trajectory = 0
                    _req.trajectory_info["valid_trajectory"] = 0
                    _req.trajectory_info["termination_reason"] = "generation_timeout"
                    finish_reason_type = FinishReasonTypeEnum.STOP
                    _req.state = AsyncRolloutRequestStateEnum.COMPLETED
                    break
                print(f"[DEBUG][verigraph] req_id={req.request_id} turn={current_turns} after engine_call, output_len={len(output.get('text',''))}", flush=True)
                content = output["text"]
                finish_reason_type = FinishReasonTypeEnum.from_str(output["meta_info"]["finish_reason"]["type"])
                current_turns += 1

                if finish_reason_type == FinishReasonTypeEnum.LENGTH:
                    _req.add_assistant_message(self.tokenizer, content.strip())
                    _req.is_valid_trajectory = 0
                    _req.trajectory_info["valid_trajectory"] = 0
                    _req.trajectory_info["termination_reason"] = "generation_length"
                    break

                code_snippet = extract_last_code_block(content)
                if code_snippet:
                    processed_content = content
                    # 去掉</code_interpreter>后面的内容
                    if CODE_CALL_TAGS[1] in content:
                        processed_content = content.rsplit(CODE_CALL_TAGS[1], 1)[0] + CODE_CALL_TAGS[1]
                    _req.add_assistant_message(self.tokenizer, processed_content.strip())
                ...
                _req.is_valid_trajectory = 0
                _req.trajectory_info["valid_trajectory"] = 0
                _req.trajectory_info["termination_reason"] = "missing_submit"
                _req.state = AsyncRolloutRequestStateEnum.COMPLETED
                break

            if current_turns >= max_turn_limit and not _req.trajectory_info.get("submitted_answer_present", False):
                _req.is_valid_trajectory = 0
                _req.trajectory_info["valid_trajectory"] = 0
                if not _req.trajectory_info.get("termination_reason"):
                    _req.trajectory_info["termination_reason"] = "max_turns"
                finish_reason_type = FinishReasonTypeEnum.STOP

            self._finalize_verigraph_trajectory_info(_req)
            _req.finalize(self.tokenizer, {}, finish_reason_type)
            return _req
        finally:
    ...
    async def _async_rollout_a_request(
        ...
        **kwargs,
        ...
        return await self._async_rollout_a_request_verigraph(req, do_sample, is_validate, max_turns, **kwargs)
    ...
    async def _handle_engine_call(self, _req: AsyncRolloutRequest, do_sample: bool, is_validate: bool, override_n: bool = True, **kwargs) -> dict:
        ...
        # For verigraph multi-turn mode, cap per-turn tokens to avoid wasting
        # GPU time generating excessively long single-turn outputs.
        per_turn_limit = self.config.response_length
        if self._is_verigraph_mode():
            verigraph_cfg = self.config.multi_turn.get("verigraph", {})
            per_turn_limit = int(verigraph_cfg.get("per_turn_max_new_tokens", 4096))
        max_new_tokens = min(per_turn_limit, self.config.max_model_len - len(generation_prompt_ids) - 1)
        if not do_sample:
            kwargs = dict(
                n=1,
                presence_penalty=0.0,
        ...
        if "n" not in kwargs or (kwargs["n"] > 1 and override_n):  # group size is supported in preprocess
            kwargs["n"] = 1
        # users can customize different sampling_params at different run
        print(f"[DEBUG][engine_call] req_id={_req.request_id} prompt_len={len(generation_prompt_ids)} max_new_tokens={max_new_tokens} sampling_params_keys={list(kwargs.keys())}", flush=True)
        with self.update_sampling_params(**kwargs):
            print(f"[DEBUG][engine_call] req_id={_req.request_id} calling async_generate...", flush=True)
            try:
                output = await asyncio.wait_for(
                    ...
                    timeout=600,  # 10-minute safety timeout per generation call
                )
            except asyncio.TimeoutError:
                print(f"[WARNING][engine_call] req_id={_req.request_id} async_generate timed out after 600s", flush=True)
                raise
            print(f"[DEBUG][engine_call] req_id={_req.request_id} async_generate returned!", flush=True)
        return output
    ...
    def _req_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if self._tp_rank == 0:
            ...
            print(f"[DEBUG][rollout] tp_rank=0, num_requests={len(req_list)}, n={1 if is_validate else self.config.n}", flush=True)
            loop = self._engine.get_event_loop()
            print(f"[DEBUG][rollout] starting asyncio.gather for {len(req_list)} requests", flush=True)
            output_req_list = loop.run_until_complete(
                asyncio.gather(
                    *[self._async_rollout_a_request(req, do_sample, is_validate, max_turns, **kwargs) for req in req_list],
                )
            )
            print(f"[DEBUG][rollout] asyncio.gather done, num_outputs={len(output_req_list)}", flush=True)
        ...
        for req in sorted_output_req_list:
            ...
            termination_reason = str((req.trajectory_info or {}).get("termination_reason", "") or "")
            train_valid_trajectory = req.is_valid_trajectory
            if termination_reason in NEGATIVE_TERMINATION_REASONS_FOR_TRAINING:
                train_valid_trajectory = 1
            valid_trajectory.append(torch.tensor(train_valid_trajectory, dtype=torch.int, device=tgt_device))

            trajectory = [{"role": msg.role, "content": msg.content} for msg in req.messages]
            messages.append(trajectory)
            trajectories.append(trajectory)
            trajectory_infos.append(dict(req.trajectory_info or {}))
        ...
        return DataProto(
            batch=batch,
            non_tensor_batch={
                "messages": _to_1d_object_array(messages),
                "trajectory": _to_1d_object_array(trajectories),
                "trajectory_info": _to_1d_object_array(trajectory_infos),
                "reward_scores": _to_1d_object_array(reward_scores),
            },
        )
    ...
    def _preprocess_prompt_to_async_rollout_requests(self, prompts: DataProto, n: int) -> list[AsyncRolloutRequest]:
        ...
        for data_idx, raw_prompt in enumerate(prompts.non_tensor_batch["raw_prompt"]):
            for rollout_offset in range(n):
                extra_info = prompts.non_tensor_batch["extra_info"][data_idx] if "extra_info" in prompts.non_tensor_batch else None
                working_dir = None
                if "working_dir" in prompts.non_tensor_batch:
                    candidate_working_dir = prompts.non_tensor_batch["working_dir"][data_idx]
                    if candidate_working_dir not in (None, ""):
                        working_dir = str(candidate_working_dir)
                if working_dir is None and isinstance(extra_info, dict):
                    for key in ("working_dir", "context_dir"):
                        candidate_working_dir = extra_info.get(key)
                        if candidate_working_dir not in (None, ""):
                            working_dir = str(candidate_working_dir)
                            break
                ...
                raw_messages = raw_prompt.tolist() if hasattr(raw_prompt, "tolist") else raw_prompt
                req = AsyncRolloutRequest(
                    ...
                    trajectory_info=self._build_initial_verigraph_trajectory_info(),
