...
def _collect_reward_metrics(non_tensor_batch) -> dict[str, float]:
    reward_metrics = {}
    metric_name_list = [
        'total_score',
        'answer_score',
    ...
    def _add_stats(name, arr):
        """Add mean/std/max/min for a numeric array."""
        reward_metrics[name] = float(np.mean(arr))
        if arr.size > 1:
            reward_metrics[f"{name}/std"] = float(np.std(arr))
            reward_metrics[f"{name}/max"] = float(np.max(arr))
            reward_metrics[f"{name}/min"] = float(np.min(arr))

    for metric_name in metric_name_list:
        if metric_name in non_tensor_batch:
            _add_stats(metric_name, np.asarray(non_tensor_batch[metric_name]))

    for metric_name, metric_values in non_tensor_batch.items():
        ...
        if metric_array.size == 0 or not np.issubdtype(metric_array.dtype, np.number):
            continue

        _add_stats(metric_name, metric_array)
...
class RayDAPOTrainer(RayPPOTrainer):
    ...
    def fit(self):
        ...
        workspace_prepared = self.prepare_workspace()
        print(f"[DEBUG] prepare workspace end prepared={workspace_prepared}")
        ...
        batch = None
        reward_batch = None
        accumulated_reward_extra_infos = defaultdict(list)
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            for batch_dict in self.train_dataloader:
                ...
                if "working_dir" in new_batch.non_tensor_batch:
                    non_tensor_batch_keys_to_pop.append("working_dir")
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("reward", timing_raw):
                        ...
                        # Accumulate reward extra infos across gen-batches for correct JSONL dump
                        if reward_extra_infos_dict:
                            for k, v in reward_extra_infos_dict.items():
                                accumulated_reward_extra_infos[k].extend(v if isinstance(v, list) else list(v))
                    ...
                    if not self.config.algorithm.filter_groups.enable:
                        ...
                        # --- Filter & intra-group statistics ---
                        total_groups = len(prompt_uid2metric_std)
                        dropped_groups = total_groups - len(kept_prompt_uids)
                        metrics["filter/total_groups"] = total_groups
                        metrics["filter/kept_groups"] = len(kept_prompt_uids)
                        metrics["filter/dropped_groups"] = dropped_groups
                        metrics["filter/drop_ratio"] = dropped_groups / max(total_groups, 1)

                        all_group_stds = list(prompt_uid2metric_std.values())
                        all_group_means = [np.mean(vals) for vals in prompt_uid2metric_vals.values()]
                        if all_group_stds:
                            metrics["reward/intra_group_std/mean"] = float(np.mean(all_group_stds))
                            metrics["reward/intra_group_std/max"] = float(np.max(all_group_stds))
                            metrics["reward/intra_group_std/min"] = float(np.min(all_group_stds))
                        if all_group_means:
                            metrics["reward/intra_group_mean/std"] = float(np.std(all_group_means))
                            metrics["reward/intra_group_mean/mean"] = float(np.mean(all_group_means))
                            metrics["reward/intra_group_mean/max"] = float(np.max(all_group_means))
                            metrics["reward/intra_group_mean/min"] = float(np.min(all_group_means))
                        metrics["reward/num_unique_prompts"] = total_groups
                        ...
                        if num_prompt_in_batch < prompt_bsz:
                            ...
                            metrics.update(_collect_reward_metrics(reward_batch.non_tensor_batch))
                    ...
                    if rollout_data_dir:
                        with _timer("dump_rollout_generations", timing_raw):
                            ...
                            self._dump_generations(
                                ...
                                outputs=outputs,
                                scores=scores,
                                reward_extra_infos_dict=dict(accumulated_reward_extra_infos),
                                dump_path=rollout_data_dir,
                                extra_dump_columns=self._build_dump_extra_columns(reward_batch),
                            )
