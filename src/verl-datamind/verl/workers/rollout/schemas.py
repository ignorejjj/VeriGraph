...
class AsyncRolloutRequest(BaseModel):
    ...
    working_dir: Optional[str] = None  # Per-sample workspace/context directory
    trajectory_info: Dict[str, Any] = Field(default_factory=dict)  # Extra structured info for reward/debug
    ...
    def add_user_response_messages(self, tokenizer: PreTrainedTokenizer, contents: Any) -> None:
        """
        Append one or multiple user messages and update tokenized fields incrementally.

        NOTE: Multi-turn RL training in this repo expects only system/user/assistant roles.
        """
        if not contents:
            return
        self.messages.append(Message(role="user", content=str(contents)))
        rendered = tokenizer.apply_chat_template([*BASE_CHAT_HISTORY, self.messages[-1]], add_generation_prompt=False, tokenize=False)
        content_ids = tokenizer.encode(rendered[self.base_conv_wo_gen_prompt_end_pos :], add_special_tokens=False)
        self._update_input_ids(content_ids, attention_mask=True, loss_mask=False)
