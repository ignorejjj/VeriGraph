...
class ActorRolloutRefWorker(Worker):
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs):
        ...
        print(f"[DEBUG][rank={_rank}] generate_sequences called, batch_size={prompts.batch.batch_size if hasattr(prompts.batch, 'batch_size') else 'unknown'}", flush=True)
        ...
        timing_generate = {}
        print(f"[DEBUG][rank={_rank}] Before entering rollout_sharding_manager", flush=True)
        with self.rollout_sharding_manager:
            print(f"[DEBUG][rank={_rank}] After entering rollout_sharding_manager", flush=True)
            log_gpu_memory_usage("After entering rollout sharding manager", logger=logger)

            prompts = self.rollout_sharding_manager.preprocess_data(prompts)
            print(f"[DEBUG][rank={_rank}] Before rollout.generate_sequences", flush=True)
            with _timer("generate_sequences", timing_generate):
                output = self.rollout.generate_sequences(prompts=prompts, **kwargs)
            print(f"[DEBUG][rank={_rank}] After rollout.generate_sequences", flush=True)
